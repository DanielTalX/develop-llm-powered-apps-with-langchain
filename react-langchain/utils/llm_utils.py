...
def get_llm_model(
    ...
    if model_name == LlmModelName.GPT_4O_MINI or model_name == LlmModelName.GPT_3_5_TURBO:
        return ChatOpenAI(model=model_name.value, temperature=temperature, stop=stop, callbacks=callbacks) # openai_api_key=api_key
    elif model_name == LlmModelName.LLAMA_3_1:
        return ChatOllama(model=model_name.value, temperature=temperature, stop=stop, callbacks=callbacks)
    else:
